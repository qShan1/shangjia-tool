...
class SelfSendDedup:
    ...
    def _gc_locked(self, now: float) -> None:
        expired = [
            k for k, entry in self._marks.items()
            if now - (entry[0] if isinstance(entry, tuple) else entry) > self._ttl
        ]
        for k in expired:
    ...
    def mark(self, cookie_id: str, chat_id: str, sender_id: str, content: str, *, pause_on_consume: bool = True) -> None:
        now = time.time()
        with self._lock:
            self._gc_locked(now)
            self._marks[self._key(cookie_id, chat_id, sender_id, content)] = (now, bool(pause_on_consume))

    def mark_auto(self, cookie_id: str, chat_id: str, sender_id: str, content: str) -> None:
        """标记自动发送回推；回推不应触发人工接管暂停。"""
        self.mark(cookie_id, chat_id, sender_id, content, pause_on_consume=False)
    ...
    def consume_with_policy(self, cookie_id: str, chat_id: str, sender_id: str, content: str) -> tuple[bool, bool]:
        """消费回推标记并返回(是否命中, 是否应暂停会话)。"""
        now = time.time()
        key = self._key(cookie_id, chat_id, sender_id, content)
        with self._lock:
            self._gc_locked(now)
