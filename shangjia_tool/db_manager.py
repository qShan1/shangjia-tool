...
© 2026 SHANGJIA TOOL"""
            ...
            try:
                ...
                if existing:
                    ...
                    if order_status is not None:
                        # External history sync can omit status and report `unknown`.
                        # Never downgrade a known status to unknown.
                        if normalized_order_status and normalized_order_status != 'unknown':
                            merged_order_status = self.resolve_external_order_status(
                                existing_order_status,
                                normalized_order_status,
                                source='insert_or_update_order',
                            )
                            if merged_order_status and merged_order_status != self._normalize_order_status(existing_order_status):
                                update_fields.append("order_status = ?")
                                update_values.append(merged_order_status)
                        elif not existing_order_status or self._normalize_order_status(existing_order_status) == 'unknown':
                            update_fields.append("order_status = ?")
    ...
    def get_pending_red_flower_orders(self, cookie_id: str, limit: int = 5,
                                      days: int = 10, cooldown_minutes: int = 30,
                                      retry_failed: bool = False,
                                      min_age_minutes: int = 10,
                                      buyer_cooldown_days: int = 30) -> List[Dict]:
        """获取待自动求小红花订单。"""
        with self.lock:
            try:
                cursor = self.conn.cursor()
                # 自动流程是订单级一次性动作。失败也不重试，避免后台轮询反复触达同一买家。
                attempt_filter = "" if retry_failed else """
                  AND NOT EXISTS (
                      SELECT 1 FROM scheduled_red_flower_logs l
                      WHERE l.order_id = o.order_id
                        AND l.cookie_id = o.cookie_id
                  )
                """
                buyer_filter = "" if buyer_cooldown_days <= 0 else f"""
                  AND (
                    o.buyer_id IS NULL OR o.buyer_id = '' OR NOT EXISTS (
                      SELECT 1 FROM scheduled_red_flower_logs bl
                      WHERE bl.cookie_id = o.cookie_id
                        AND bl.buyer_id = o.buyer_id
                        AND bl.status = 'success'
                        AND datetime(bl.created_at) >= datetime('now', '-{max(1, int(buyer_cooldown_days))} days')
                    )
                  )
                """
                cursor.execute(f'''
                ...
                WHERE o.cookie_id = ?
                  ...
                  {attempt_filter}
                  {buyer_filter}
                ORDER BY datetime(COALESCE(o.platform_completed_at, o.updated_at, o.created_at)) ASC
                LIMIT ?
                ''', (
                    cookie_id,
                    f'-{max(1, int(days or 10))} days',
                    f'-{max(0, int(min_age_minutes or 0))} minutes',
    ...
    def get_red_flower_attempt(self, order_id: str, cookie_id: str = None) -> Optional[Dict]:
        """返回订单已有的自动求小红花记录，用于防止并发重复请求。"""
        with self.lock:
            try:
                conditions = ["order_id = ?"]
                params = [str(order_id or "").strip()]
                if cookie_id:
                    conditions.append("cookie_id = ?")
                    params.append(str(cookie_id).strip())
                cursor = self.conn.cursor()
                cursor.execute(f"""
                    SELECT id, status, message, created_at
                    FROM scheduled_red_flower_logs
                    WHERE {' AND '.join(conditions)}
                    ORDER BY id ASC
                    LIMIT 1
                """, params)
                ...
                if not row:
                    return None
                return {'id': row[0], 'status': row[1], 'message': row[2], 'created_at': row[3]}
            except Exception as e:
                logger.error(f"查询订单小红花记录失败: order_id={order_id}, error={e}")
                return None
