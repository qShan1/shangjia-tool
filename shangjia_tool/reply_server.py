...
async def start_scheduled_task_checker():
    ...
    try:
        await ai_site_audit_service.restore_schedule()
        logger.info("AI系统定时巡检配置已恢复")
    except Exception as exc:
        logger.error(f"AI系统定时巡检恢复失败: {exc}")
    try:
...
async def get_sales_data(
    ...
    try:
        ...
        query = (
            ...
            "AND EXISTS (SELECT 1 FROM item_info i WHERE i.cookie_id = o.cookie_id AND i.item_id = o.item_id) "
...
async def get_sales_summary(
    ...
    try:
        ...
        query = (
            f"SELECT amount, {ORDER_SALES_TIME_SQL} AS effective_sales_at, order_status "
            f"FROM orders o JOIN cookies c ON c.id = o.cookie_id WHERE {ORDER_SALES_TIME_SQL} >= ? AND o.cookie_id IN ({placeholders}) "
            "AND EXISTS (SELECT 1 FROM item_info i WHERE i.cookie_id = o.cookie_id AND i.item_id = o.item_id) "
            "AND NOT (TRIM(COALESCE(o.buyer_nick, '')) <> '' AND TRIM(COALESCE(c.remark, '')) <> '' "
            "AND TRIM(o.buyer_nick) = TRIM(c.remark))"
        )
...
class AISiteAuditStartRequest(BaseModel):
...
def _is_account_risk_protected_status(status_note: Any) -> bool:
    """Return whether an account is intentionally blocked after platform risk control."""
    normalized = str(status_note or '').strip().lower()
    return (
        normalized == 'account_risk_protected'
        or 'account_risk_protected' in normalized
        or '滑块验证失败，平台风控保护中' in str(status_note or '')
    )


def _build_live_runtime_status(cookie_id: str) -> Dict[str, Any]:
    cleaned_cid = str(cookie_id or '').strip()
    cookie_details = db_manager.get_cookie_details(cleaned_cid) if cleaned_cid else None
    status_note = (cookie_details or {}).get('status_note') or ''
    account_enabled = bool((cookie_details or {}).get('enabled', db_manager.get_cookie_status(cleaned_cid) if cleaned_cid else False))
    risk_protected = _is_account_risk_protected_status(status_note)
    runtime_status = {
        'account_enabled': account_enabled,
        'status_note': status_note,
        'risk_protected': risk_protected,
        'recovery_action_required': risk_protected,
        'order_monitoring_available': False,
        'order_fallback_required': risk_protected,
    ...
    password_login_backoff_remaining = None
    try:
        backoff_state = XianyuLive.get_password_login_failure_backoff(cleaned_cid)
        if backoff_state:
            password_login_backoff_remaining = max(0.0, float(backoff_state.get('until', 0) or 0) - now)
            if password_login_backoff_remaining <= 0:
                password_login_backoff_remaining = None
    except Exception:
        password_login_backoff_remaining = None
    ...
    # A prior authentication failure can remain in cookies.status_note after
    # the live instance has recovered. Do not expose that historical note as
    # the current account state when all active transport signals are healthy.
    current_runtime_healthy = bool(
        ws_ready and session_ready and token_ready and message_stream_ready
    )
    ...
    runtime_status.update({
        ...
        'risk_protected': risk_protected,
        'recovery_action_required': risk_protected,
        'order_monitoring_available': bool(message_stream_ready and account_enabled and not risk_protected),
        'order_fallback_required': bool(risk_protected or not message_stream_ready),
        ...
        'token_refresh_error_message': (
            f"密码登录失败退避中，剩余{password_login_backoff_remaining:.1f}秒"
            if password_login_backoff_remaining is not None
            else getattr(live_instance, 'last_token_refresh_error_message', None)
        ),
        'password_login_backoff_remaining_seconds': password_login_backoff_remaining,
    ...
    *,
    ...
    try:
        if timeout and timeout > 0:
            # Do not use wait_for here: cancelling a cross-loop Future can wait
            # for the manager-loop coroutine to unwind, defeating the timeout.
            done, _ = await asyncio.wait({wrapped_future}, timeout=timeout)
            if not done:
...
def get_cookies_details(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    for cookie_id, cookie_value in user_cookies.items():
        ...
        result.append({
            ...
            'remark': remark,
            'status_note': runtime_status.get('status_note', status_note),
            'username': username,
...
def _get_latest_risk_log_epoch_for_account(account_id: str) -> Optional[float]:
    """Return the newest parseable risk-log timestamp in UTC."""
    logs = db_manager.get_risk_control_logs(cookie_id=str(account_id), limit=20) or []
    timestamps = []
    for log in logs:
        if not isinstance(log, dict):
            continue
...
def _evaluate_screenshot_freshness(
    screenshot_path: str,
    latest_risk_log_epoch: Optional[float],
) -> Tuple[str, Optional[str]]:
    """Classify a verification screenshot without treating a missing file as stale."""
    try:
        screenshot_mtime = os.path.getmtime(screenshot_path)
    except (FileNotFoundError, OSError):
        return 'unavailable', '验证截图已不存在或暂时不可用'

    if latest_risk_log_epoch is None:
        return 'ok', None

    age_after_risk = latest_risk_log_epoch - screenshot_mtime
    if age_after_risk <= _SCREENSHOT_STALE_GAP_SECONDS:
        return 'ok', None
    return 'stale', '验证截图早于最新风控记录，已过期'
...
async def _execute_manual_cookie_import(
    ...
    try:
        ...
        def persist_manual_cookie_import_success(merged_cookies_dict: Dict[str, Any], source_label: str):
            ...
            # Only a successful manual import/preflight can release the risk-protection lock.
            db_manager.update_cookie_status_note(account_id, '')
...
async def password_login(
    ...
    try:
        ...
        if refresh_mode and account_id:
            ...
            backoff_state = XianyuLive.get_password_login_failure_backoff(account_id)
            if backoff_state:
                remaining_seconds = max(0.0, float(backoff_state.get('until', 0) or 0) - time.time())
                if remaining_seconds > 0:
                    return {
                        'success': False,
                        'message': f'账号当前处于闲鱼登录退避中，剩余{remaining_seconds:.1f}秒，暂不能手动刷新Cookie，请退避结束后再试',
                    }
...
async def get_account_face_verification_screenshot(
    ...
    try:
        ...
        # Never expose an old screenshot as a currently actionable verification.
        # A newer slider/token/password event means any previous face/QR screenshot
        # is stale until a new verification session is created.
        latest_account_logs = db_manager.get_risk_control_logs(cookie_id=account_id, limit=1)
        if latest_account_logs:
            latest_account_log = latest_account_logs[0]
            latest_event_type = str(latest_account_log.get('event_type') or '').strip()
            latest_processing_status = str(latest_account_log.get('processing_status') or '').strip().lower()
            if latest_event_type not in {'qr_verify', 'face_verify', 'sms_verify', 'unknown'} \
                    or latest_processing_status != 'processing':
                return {
                    'success': False,
                    'message': '当前没有待处理的验证，历史验证截图已失效'
                }
...
def update_cookie_status(cid: str, status_data: CookieStatusIn, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        cookie_details = db_manager.get_cookie_details(cid) or {}
        status_note = cookie_details.get('status_note') or ''
        if status_data.enabled and _is_account_risk_protected_status(status_note):
            raise HTTPException(
                status_code=409,
                detail=(
...
@app.get('/api/ai-site-audit/status')
async def get_ai_site_audit_status(current_user: Dict[str, Any] = Depends(get_current_user)):
    return ai_site_audit_service.get_status()


@app.post('/api/ai-site-audit/start')
async def start_ai_site_audit(
    request: AISiteAuditStartRequest,
    current_user: Dict[str, Any] = Depends(get_current_user),
):
    if not current_user.get('is_admin'):
        raise HTTPException(status_code=403, detail='仅管理员可启动AI系统巡检')
    if request.interval_hours < ai_site_audit_service.MIN_INTERVAL_HOURS or request.interval_hours > ai_site_audit_service.MAX_INTERVAL_HOURS:
        raise HTTPException(status_code=400, detail='巡检周期需在1-168小时之间')
    try:
        return await ai_site_audit_service.start(
...
@app.post('/api/ai-site-audit/stop')
async def stop_ai_site_audit(current_user: Dict[str, Any] = Depends(get_current_user)):
    if not current_user.get('is_admin'):
        raise HTTPException(status_code=403, detail='仅管理员可停止AI系统巡检')
    return await ai_site_audit_service.stop()


@app.get('/api/ai-site-audit/reports')
async def list_ai_site_audit_reports(current_user: Dict[str, Any] = Depends(get_current_user)):
    return {'reports': ai_site_audit_service.list_reports()}


@app.get('/api/ai-site-audit/reports/{report_id}')
async def get_ai_site_audit_report(report_id: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    if not re.fullmatch(r'[a-f0-9]{32}', report_id or ''):
        raise HTTPException(status_code=400, detail='报告编号无效')
    report = ai_site_audit_service.get_report(report_id)
    if not report:
...
def update_cookie_pause_duration(cid: str, update_data: PauseDurationUpdate, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        if success:
            if update_data.pause_duration == 0:
                # 0 means disable chat pausing immediately, including pauses
                # already held in the running process memory.
                from XianyuAutoAsync import pause_manager
                pause_manager.clear_cookie_pauses(cid)
    ...
    try:
        ...
        for raw_cookie_id in target_cookie_ids:
            ...
            for order in orders:
                ...
                if result.get('success'):
                    stats['success'] += 1
                elif result.get('status') in {'skipped', 'already_red_flower', 'already_attempted'}:
                    stats['skipped'] += 1
                else:
                    stats['failed'] += 1
                await asyncio.sleep(1)
...
async def connect_chat_account(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        cookie_details = db_manager.get_cookie_details(cid) or {}
        status_note = cookie_details.get('status_note') or ''
        if _is_account_risk_protected_status(status_note):
            raise HTTPException(
                status_code=409,
                detail=(
            ...
            if live_instance:
                ...
                try:
                    ...
                    if isinstance(body, dict) and (body.get('reason') or body.get('code')) and not body.get('userMessageModels'):
                        ...
                        # IM can return an empty page while the account is reconnecting.
                        # Keep the operator's history visible from the local cache.
                        cached_messages = db_manager.get_chat_messages(
                            cookie_id,
                            normalized_chat_id,
                            limit=normalized_limit,
                            before_id=before_id,
                        ...
                        if cached_messages:
                            return {
                                ...
                                'source': 'local_cache',
                                'remote_error': '远程消息暂未返回，当前显示本地缓存',
                                'has_more': len(cached_messages) >= normalized_limit,
                                'next_cursor': None,
                            }
