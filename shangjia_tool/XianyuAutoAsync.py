...
# 全局浏览器并发上限：同时最多允许 MAX 个 chromium 实例，防止刷新/扫码/搜索叠加抢占资源
...
def _force_kill_chromium(pid=None):
    ...
    优先按浏览器进程 PID 精确清理，避免误杀用户本机其它 chrome。
    """
    try:
        if pid:
            subprocess.run(
        ...
        logger.warning(f"强杀chromium进程树失败: {e}")
...
class AutoReplyPauseManager:
    ...
    def pause_chat_for(self, chat_id: str, cookie_id: str, pause_minutes: int, reason: str = "消息过滤规则"):
        ...
        if pause_minutes <= 0:
            ...
            self.paused_chats.pop(chat_id, None)
            self.paused_chat_cookies.pop(chat_id, None)
            return
        ...
        self.paused_chat_cookies[chat_id] = str(cookie_id)
    ...
    def clear_cookie_pauses(self, cookie_id: str) -> int:
        """立即清理指定账号已有的聊天暂停记录。"""
        ...
        chat_ids = [
            chat_id for chat_id, owner in self.paused_chat_cookies.items()
            if str(owner).strip() == normalized_cookie_id
        ]
        for chat_id in chat_ids:
            self.paused_chats.pop(chat_id, None)
            self.paused_chat_cookies.pop(chat_id, None)
        if chat_ids:
            logger.info(f"【{normalized_cookie_id}】已清理 {len(chat_ids)} 个聊天的自动回复暂停记录")
        return len(chat_ids)
...
class XianyuLive:
    ...
    async def _protect_account_from_slider_failure(self, error_message: str) -> bool:
        """Stop automatic recovery after a platform slider failure."""
        if self._is_account_pause_status(getattr(self, 'last_token_refresh_status', None)):
            return True

        message = str(error_message or "Slider verification failed").strip()
        await self._apply_account_pause_state(
            refresh_status="account_risk_protected",
            status_note="\u6ed1\u5757\u9a8c\u8bc1\u5931\u8d25\uff0c\u5e73\u53f0\u98ce\u63a7\u4fdd\u62a4\u4e2d",
            error_message=(
                f"{message}. \u5e73\u53f0\u6ed1\u5757\u9a8c\u8bc1\u672a\u901a\u8fc7\uff0c\u7cfb\u7edf\u5df2\u505c\u6b62\u81ea\u52a8\u767b\u5f55\u91cd\u8bd5\uff0c"
                "\u8bf7\u7b49\u5f85\u95f2\u9c7c\u5b98\u65b9\u7aef\u98ce\u63a7\u89e3\u9664\u540e\u518d\u6062\u590d\u3002"
            ),
            connection_message="\u6ed1\u5757\u9a8c\u8bc1\u5931\u8d25\uff0c\u5df2\u505c\u6b62\u81ea\u52a8\u6062\u590d",
            note_error_prefix="\u5199\u5165\u6ed1\u5757\u98ce\u63a7\u72b6\u6001\u6587\u6848\u5931\u8d25",
            status_error_prefix="\u6301\u4e45\u5316\u6ed1\u5757\u98ce\u63a7\u72b6\u6001\u5931\u8d25",
            memory_error_prefix="\u66f4\u65b0\u6ed1\u5757\u98ce\u63a7\u5185\u5b58\u72b6\u6001\u5931\u8d25",
        )
        await self.send_account_paused_notification(
            status_note="\u6ed1\u5757\u9a8c\u8bc1\u5931\u8d25\uff0c\u5e73\u53f0\u98ce\u63a7\u4fdd\u62a4\u4e2d",
            pause_reason="\u6ed1\u5757\u9a8c\u8bc1\u5931\u8d25",
            error_message=(
                "\u6ed1\u5757\u9a8c\u8bc1\u672a\u901a\u8fc7\uff0c\u7cfb\u7edf\u5df2\u505c\u6b62\u81ea\u52a8\u767b\u5f55\u91cd\u8bd5\u3002"
                "\u8fd9\u4e0d\u662f\u7b49\u5f85\u4eba\u5de5\u6ed1\u5757\uff0c\u8bf7\u7b49\u5f85\u95f2\u9c7c\u5b98\u65b9\u7aef\u98ce\u63a7\u89e3\u9664\u540e\u518d\u6062\u590d\u3002"
            ),
            verification_url='',
        )
        await self._request_stop_after_account_pause("slider failure triggered platform risk protection")
        return True
    ...
    def __init__(self, cookies_str=None, cookie_id: str = "default", user_id: int = None, *, register_instance: bool = True):
        ...
        # 给买家留出连续补充两三条消息的时间，单条消息仍会在窗口结束后回复。
        self._message_debounce_delay = 8  # 防抖延迟默认值（秒），实际值通过property从数据库动态读取
    ...
    def _record_delivery_log(self, order_id: str = None, item_id: str = None, buyer_id: str = None,
        ...
        try:
            ...
            # Missing delivery rules are a configuration state, not a
            # transient delivery failure. Keep one audit record per order so
            # repeated platform notifications do not flood the log.
            is_blocked_no_rule = (
                normalized_status == 'failed'
                and (
                    str(meta.get('match_mode') or '').strip().lower().startswith('blocked_no_rule')
                    or '未找到匹配' in str(reason or '')
                )
            )
            if is_blocked_no_rule and order_id:
                with db_manager.lock:
                    cursor = db_manager.conn.cursor()
                    cursor.execute(
                        """SELECT 1 FROM delivery_logs
                           WHERE order_id = ? AND status = 'failed'
                             AND match_mode = 'blocked_no_rule'
                           LIMIT 1""",
                        (order_id,)
                    )
                    if cursor.fetchone():
    ...
    def _classify_message_route(self, *, message: dict, message_1: dict, message_10: dict,
        ...
        if send_message in special_flow_messages or card_title in special_flow_titles:
            ...
            if is_system_message and order_status_signal:
                route = 'order_status'
            elif send_message in platform_notice_exact:
                route = 'system_notice'
    ...
    async def preflight_token_after_manual_refresh(self) -> str:
        ...
        for attempt in range(1, max_preflight_retries + 1):
            ...
            if self.last_token_refresh_status == 'password_login_backoff_wait':
                raise InitAuthError(
                    '闲鱼登录退避中，当前不能完成手动刷新，请等待退避结束后再试'
                )
                    ...
                    if self._need_captcha_verification(res_json):
                        ...
                        try:
                            ...
                            if new_cookies_str:
                                ...
                                await self._protect_account_from_slider_failure(
                                    self.last_token_refresh_error_message
                                )
    ...
    async def _try_password_login_refresh(
        ...
        try:
            ...
            if result:
                ...
                if backoff_reason == 'slider_failed':
                    await self._protect_account_from_slider_failure(login_error)
                    return False
            ...
            XianyuLive.set_password_login_failure_backoff(self.cookie_id, backoff_reason, backoff_seconds)
            if backoff_reason == 'slider_failed':
                await self._protect_account_from_slider_failure(str(refresh_e))
                return False
            protected = await self._protect_account_for_consecutive_failures(
                XianyuLive.get_password_login_failure_backoff(self.cookie_id)
            )
            ...
            if chat_id:
                with db_manager.lock:
                    ...
                    CREATE TABLE IF NOT EXISTS item_reply_records (
                        ...
                        UNIQUE(cookie_id, item_id, chat_id)
                    )
                    ''')
                    cursor.execute('''
                    INSERT OR IGNORE INTO item_reply_records (cookie_id, item_id, chat_id)
                    SELECT cookie_id, item_id, chat_id
                    FROM chat_messages
                    WHERE direction = 1 AND reply_source = ?
                    ...
                    INSERT OR IGNORE INTO item_reply_records (cookie_id, item_id, chat_id)
                    VALUES (?, ?, ?)
                    ''', (self.cookie_id, item_id, chat_id))
                    claimed = cursor.rowcount > 0
                    db_manager.conn.commit()
                if not claimed:
                    logger.info(f"[{self.cookie_id}] chat {chat_id} already received item {item_id} reply")
                    return None
    ...
    async def get_default_reply(self, send_user_name: str, send_user_id: str, send_message: str, chat_id: str, item_id: str = None) -> str:
        ...
        try:
            ...
            try:
                ...
                if default_reply_settings.get('reply_once', False) and chat_id:
                    # The once-only marker is written only after the message is sent.
    ...
    async def get_ai_reply(self, send_user_name: str, send_user_id: str, send_message: str, item_id: str, chat_id: str):
        ...
        try:
            ...
            # 图片消息先落库，再从会话记录取回，避免改动现有消息解析链。
            image_url = None
            ai_message = send_message
            try:
                recent_chat_messages = db_manager.get_chat_messages(self.cookie_id, chat_id, limit=20)
                for chat_message in reversed(recent_chat_messages or []):
                    if chat_message.get('direction') == 2 and chat_message.get('image_url'):
                        image_url = chat_message['image_url']
                        break
                # 防抖结束后，把最近一次卖家回复之后的买家文字合并给 AI，
                # 避免买家连续补充问题时 AI 只看到最后一句。
                pending_buyer_messages = []
                for chat_message in reversed(recent_chat_messages or []):
                    direction = int(chat_message.get('direction') or 0)
                    if direction == 1:
                        break
                    if direction == 2:
                        content = str(chat_message.get('content') or '').strip()
                        if content:
                            pending_buyer_messages.append(content)
                pending_buyer_messages.reverse()
                if pending_buyer_messages:
                    ai_message = '\n'.join(pending_buyer_messages[-6:])
                    if len(pending_buyer_messages) > 1:
                        logger.info(
                            f"【{self.cookie_id}】AI收集到{len(pending_buyer_messages[-6:])}条连续买家消息，合并后生成回复"
                        )
            except Exception as image_error:
                logger.warning(f"获取客户图片用于AI识别失败，继续文本回复: {self._safe_str(image_error)}")
        ...
        try:
            while True:
                try:
                    ...
                    active_login_backoff = self._get_active_password_login_failure_backoff(current_time)
                    if active_login_backoff:
                        remaining_backoff = max(1, int(active_login_backoff.get('remaining_time', 0) or 0))
                        logger.info(f"account backoff active; retrying after {remaining_backoff}s")
                        await self._interruptible_sleep(min(60, remaining_backoff + 1))
                        continue

                    # Once the backoff expires, allow one controlled recovery attempt
                    # immediately instead of waiting for the normal long refresh interval.
                    recovery_retry_due = self.last_token_refresh_status == 'password_login_backoff_wait'
                    if self._should_skip_token_refresh_for_login_backoff(current_time):
                    ...
                    if recovery_retry_due or current_time - self.last_cookie_refresh_time >= effective_cookie_refresh_interval:
                        # 检查是否在消息接收后的冷却时间内
                        time_since_last_message = current_time - self.last_message_received_time
    ...
    async def refresh_cookies_from_qr_login(self, qr_cookies_str: str, cookie_id: str = None, user_id: int = None):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"【{target_cookie_id}】已释放浏览器并发槽位")
            except Exception as sem_e:
                logger.warning(f"【{target_cookie_id}】释放浏览器并发槽位失败: {self._safe_str(sem_e)}")
            # 确保资源清理
            try:
                ...
                if playwright:
                    try:
                        ...
                        logger.warning(f"【{target_cookie_id}】Playwright关闭超时（2秒），强杀chromium进程树")
                        _force_kill_chromium(browser_pid)
                        # 尝试清理Playwright的内部状态
                        try:
    ...
    async def _refresh_cookies_via_browser_page(self, current_cookies_str: str, restart_on_success: bool = True):
        ...
        try:
            ...
            try:
                _BROWSER_SEMAPHORE.release()
                logger.info(f"【{self.cookie_id}】已释放浏览器并发槽位")
            except Exception as sem_e:
                logger.warning(f"【{self.cookie_id}】释放浏览器并发槽位失败: {self._safe_str(sem_e)}")
            # 确保资源清理
            try:
                # 先关闭浏览器，再关闭Playwright（顺序很重要）
                if browser:
                    try:
                        ...
                        logger.warning(f"【{self.cookie_id}】浏览器关闭超时（5秒），强杀chromium进程树")
                        _force_kill_chromium(browser_pid)
                    except Exception as e:
                ...
                if playwright:
                    try:
                        ...
                        logger.warning(f"【{self.cookie_id}】Playwright关闭超时（2秒），强杀chromium进程树")
                        _force_kill_chromium(browser_pid)
                    except Exception as e:
            ...
            try:
                if browser or playwright:
                    ...
                    close_task = asyncio.create_task(
                        self._async_close_browser(browser, playwright, browser_pid)
                    )
    ...
    async def _force_close_resources(self, browser, playwright, browser_pid=None):
            ...
            if force_tasks:
                ...
                for i, result in enumerate(results):
                    if isinstance(result, (asyncio.TimeoutError, Exception)):
                        ...
                        logger.warning(f"【{self.cookie_id}】{resource_name}强制关闭失败，强杀chromium进程树")
                        _force_kill_chromium(browser_pid)
                        try:
    ...
    async def _schedule_debounced_reply(self, chat_id: str, message_data: dict, websocket,
        ...
        self.chat_latest_activity_at[chat_id] = activity_at
        ...
        async with self.message_debounce_lock:
            ...
            async def debounce_task():
                ...
                try:
                    # 每次新消息都会取消并重启计时，因此这是“最后一条消息后的等待窗口”。
                    ...
                    if (time.monotonic() - last_msg['activity_at']) > self.message_expire_seconds:
                        logger.info(f"【{self.cookie_id}】chat_id {chat_id} 的回复任务已过期，跳过旧消息")
                        await self._finalize_message_reply(last_msg['message_id'], reason="回复任务过期")
                        return
                    if self.chat_latest_activity_at.get(chat_id, last_msg['activity_at']) > last_msg['activity_at']:
                        logger.info(f"【{self.cookie_id}】chat_id {chat_id} 在回复生成期间有新消息，跳过旧回复")
                        await self._finalize_message_reply(last_msg['message_id'], reason="会话已有新消息")
                        return
    ...
    def _should_suppress_low_signal_auto_reply(self, send_message: str, message_data: dict) -> bool:
        """过滤确认、等待和无内容消息，避免 AI/默认回复打扰买家。"""
        text = re.sub(r'\s+', '', str(send_message or '')).strip()
        if not text:
            return True
        if self._extract_image_url_from_message(message_data) and text in {'[图片]', '图片'}:
            return False
        low_signal = {
            'ok', 'ok吗', '好的', '好', '嗯', '嗯嗯', '对', '是的', '收到',
            '1', '？', '?', '。', '等下啊', '等我一下', '然后捏', '行', '可以',
        }
        return text.lower() in low_signal
    ...
    async def _process_chat_message_reply(self, message_data: dict, websocket, send_user_name: str,
                                         ...
                                         chat_id: str, msg_time: str, activity_at: float = 0.0):
        """
        ...
        Args:
            ...
            msg_time: 消息时间
        """
        activity_at = activity_at or self.chat_latest_activity_at.get(chat_id, time.monotonic())
        try:
            # 自动回复消息
            ...
            if self._should_suppress_low_signal_auto_reply(send_message, message_data):
                logger.info(f"[{msg_time}] 【{self.cookie_id}】低信息量消息不触发自动回复: {send_message}")
                return
            ...
            # 回复优先级：指定商品首条回复 > 关键词 > AI > 默认回复兜底。
            # 默认回复不能放在 AI 前面，否则启用 AI 后永远不会调用 AI。
            ...
            if reply:
                # 检查是否是图片发送标记
                if self.chat_latest_activity_at.get(chat_id, activity_at) > activity_at:
                    logger.info(f"[{msg_time}] 【{self.cookie_id}】会话已有新活动，跳过旧的自动回复")
                    return
                if pause_manager.is_chat_paused(chat_id):
                    logger.info(f"[{msg_time}] 【{self.cookie_id}】会话在生成回复期间被暂停，跳过自动回复")
                    return
                if reply.startswith("__IMAGE_SEND__"):
                    ...
                    if reply_source == '默认' and chat_id:
                        from db_manager import db_manager as _reply_db
                        _reply_db.add_default_reply_record(self.cookie_id, chat_id)
                    try:
                        from chat_event_hub import self_send_dedup
                        self_send_dedup.mark_auto(self.cookie_id, chat_id, str(self.myid), reply)
                    except Exception as _mark_error:
                        logger.debug(f"自动消息回推去重标记失败: {self._safe_str(_mark_error)}")
                    # 记录发出的消息
                    msg_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    ...
    async def handle_message(self, message_data, websocket, msg_id="unknown"):
        ...
        try:
            ...
            if send_user_id == self.myid and not is_system_message:
                self.chat_latest_activity_at[chat_id] = time.monotonic()
                ...
                try:
                    from chat_event_hub import self_send_dedup
                    matched, should_pause = self_send_dedup.consume_with_policy(
                        self.cookie_id, chat_id, str(self.myid), send_message
                    )
                    if matched:
                        if should_pause:
                            pause_manager.pause_chat(chat_id, self.cookie_id)
                        else:
                            logger.info(f"【{self.cookie_id}】自动消息回推已去重，不触发人工暂停")
