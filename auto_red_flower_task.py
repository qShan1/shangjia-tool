...
DEFAULT_MIN_AGE_MINUTES = int(os.getenv("AUTO_RED_FLOWER_TASK_MIN_AGE_MINUTES", "10") or 10)
DEFAULT_BUYER_COOLDOWN_DAYS = int(os.getenv("AUTO_RED_FLOWER_TASK_BUYER_COOLDOWN_DAYS", "30") or 30)
...
async def request_red_flower_once(
    ...
    *,
    ...
    # 定时流程和手动批次共享订单级锁；明确的单订单手动操作仍可绕过。
    if source in {"scheduled_red_flower", "manual_batch"}:
        previous_attempt = db_manager.get_red_flower_attempt(order_id, cookie_id)
        if previous_attempt:
            message = "该订单已处理过小红花请求，自动流程不再重复"
            return {
                "success": False,
                "message": message,
    ...
    if str(order_info.get("order_status") or "").strip().lower() != "completed":
        message = "订单尚未明确完成，暂不请求小红花"
        db_manager.add_scheduled_red_flower_log(
            batch_id, cookie_id, order_id=order_id, item_id=order_info.get("item_id"),
            buyer_id=order_info.get("buyer_id"), buyer_nick=order_info.get("buyer_nick"),
            status="skipped", message=message,
...
async def run_auto_red_flower_batch(
    *,
    ...
    buyer_cooldown_days: int = DEFAULT_BUYER_COOLDOWN_DAYS,
) -> Dict[str, Any]:
    """执行一轮自动求小红花。"""
    ...
    for cookie_id in list(all_cookies.keys()):
        try:
            ...
            orders = db_manager.get_pending_red_flower_orders(
                ...
                buyer_cooldown_days=buyer_cooldown_days,
            )
            if not orders:
                continue
