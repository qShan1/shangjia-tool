...
class ItemPublisher:
    ...
    async def prepare_image_for_publish(self, image: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if image_url:
            # Product materials may reference the local static upload directory.
            # Those paths are not reachable by Goofish and must be uploaded first.
            local_path = ""
            if image_url.startswith("/static/"):
                local_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), image_url.lstrip("/"))
            elif image_url.startswith("http://127.0.0.1") or image_url.startswith("http://localhost"):
                from urllib.parse import urlparse
                parsed_path = urlparse(image_url).path
                if parsed_path.startswith("/static/"):
                    local_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), parsed_path.lstrip("/"))
