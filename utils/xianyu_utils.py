...
def generate_device_id(user_id: str) -> str:
    ...
    # 原逻辑手写拼接 UUID v4（8/13/18/23 加横杠、位14=4、位19=variant），
    # 等价于 str(uuid.uuid4())。用 .upper() 保持与原输出大写十六进制逐字节一致。
    return str(uuid.uuid4()).upper() + "-" + user_id
...
def decrypt(data: str) -> str:
    ...
    try:
        ...
        raise Exception(f"解密失败: {e}") from e
